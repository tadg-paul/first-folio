...
sub preamble {
    ...
    my $font_weight  = $config->get('folio.font-weight');
    my $font_stretch = $config->get('folio.font-stretch');
    my $margin       = $config->get('folio.margin')      // '25mm';
    my $page         = $config->get('folio.page')        // 'a4';

    # Build the global #set text(...) attribute list, optionally adding
    # weight/stretch so users can pin a specific variant of a font family
    # (e.g. Asap with weight: thin to avoid Typst's nearest-neighbour fallback).
    my $global_text_attrs = qq{font: "$font", size: $font_size};
    ...
    if (defined $font_stretch && $font_stretch ne '') {
        my $s = $font_stretch;
        $s =~ s/^\s+|\s+$//g;
        my $sval = $s =~ /%$/ ? $s : ($s =~ /^\d+(\.\d+)?$/ ? "${s}%" : $s);
        $global_text_attrs .= ", stretch: $sval";
    }
