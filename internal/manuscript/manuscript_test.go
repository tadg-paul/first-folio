...
func assertExampleTypstMatches(t *testing.T, root string, style string) {
	...
	for name, typst := range map[string]string{"markdown": readFile(t, mdTypst), "org": readFile(t, orgTypst)} {
		...
		assertContains(t, typst, `#emph[soft warning]`)
		assertContains(t, typst, `#strike[an erased`)
		assertContains(t, typst, `instruction]`)
		assertContains(t, typst, `#show list: it => block(`)
		assertContains(t, typst, `#show enum: it => block(`)
